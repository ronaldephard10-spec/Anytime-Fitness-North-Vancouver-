      ...
      server.middlewares.use('/api/send-inspection', async (req, res) => {
        ...
        req.on('end', async () => {
          try {
            ...
            console.error('Error processing inspection send:', err);
            res.statusCode = 500;
            res.setHeader('Content-Type', 'application/json');
  ...
  return {
    plugins: [
      ...
      VitePWA({
        ...
        manifest: {
          id: '/',
          name: 'Anytime Fitness Inspection Checklist',
          short_name: 'AF Inspect',
          description: 'Dedicated PWA inspection checklist for Anytime Fitness - North Vancouver (Northwoods Village)',
          theme_color: '#4B286D',
          background_color: '#0f0817',
          display: 'standalone',
          start_url: '/',
          scope: '/',
          icons: [
